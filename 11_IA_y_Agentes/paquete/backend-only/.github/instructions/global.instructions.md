...
- Closed sprint decisions take precedence over older kit wording when both cannot be true at the same time.
...
- Read `.github/context/current-sprint-scope.md` before implementation work.
...
- Do not implement behavior that belongs to a later sprint when the current sprint scope file marks it out of scope.
